...
| 24 | 车辆下架时，F-003 挂载的图片是否需要联动处理？ | reviewer-api（/conflicts 汇总） | ⏳ 推迟到 F-003 评审时处理，不阻塞 F-001 |
| 25 | F-005 的删除操作允许作用于哪些状态的车辆？ | reviewer-domain（/conflicts 汇总） | ⏳ 推迟到 F-005 评审时处理，不阻塞 F-001 |
